...
export default class CasosPage extends Component {
    ...
    solicitarArchivarCaso = (id) => {
        this.setState({
            confirmacionVisible: true,
            tituloConfirmar: 'Archivar Caso',
            mensajeConfirmar: '¿Estás seguro de que deseas archivar este caso? Desaparecerá de la lista principal.',
            accionConfirmar: () => this.ejecutarArchivarCaso(id)
        });
    }

    ejecutarArchivarCaso = async (id) => {
        this.cerrarConfirmacion();
        const token = localStorage.getItem('token');
        const config = { headers: { "Authorization": `Bearer ${token}` } };
        try {
            await clienteAxios.put(`/casos/archivar/${id}`, {}, config);
            this.setState({ notificacion: { mensaje: 'El caso fue archivado', tipo: 'exito' } });
            this.obtenerCasos();
        } catch (error) {
    ...
    solicitarReactivarCaso = (id) => {
        this.setState({
            confirmacionVisible: true,
            tituloConfirmar: 'Reactivar Caso',
            mensajeConfirmar: '¿Seguro que deseas reactivar este caso? Volverá a la lista de "Activos y Cerrados".',
            accionConfirmar: () => this.ejecutarReactivarCaso(id)
        });
    }

    ejecutarReactivarCaso = async (id) => {
        this.cerrarConfirmacion();
        const token = localStorage.getItem('token');
        const config = { headers: { "Authorization": `Bearer ${token}` } };
        try {
            await clienteAxios.put(`/casos/reactivar/${id}`, {}, config);
            this.setState({ notificacion: { mensaje: 'El caso fue reactivado', tipo: 'exito' } });
            this.obtenerCasos();
        } catch (error) {
                                ...
                                {casosFiltrados.map(caso => (
                                    <li className="list-group-item d-flex justify-content-between align-items-center" key={caso._id} style={{ backgroundColor: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', marginBottom: '10px', borderRadius: '8px' }}>
                                        ...
                                        <div className="ms-3 d-flex flex-column">
                                            ...
                                            <button onClick={() => this.solicitarArchivarCaso(caso._id)} className="btn btn-secondary btn-sm d-block">Archivar</button>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                                    ...
                                    <ul className="list-group">
                                        {casosArchivados.map(caso => (
                                            <li key={caso._id} className="list-group-item d-flex justify-content-between align-items-center" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', opacity: 0.7, color: 'white', border: 'none', marginBottom: '10px', borderRadius: '8px' }}>
                                                ...
                                                <div className="ms-3">
                                                    <button onClick={() => this.solicitarReactivarCaso(caso._id)} className="btn btn-success btn-sm">Reactivar</button>
                                                </div>
                                            </li>
                                        ))}
                                    </ul>
